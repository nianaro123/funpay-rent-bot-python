GOLDEN_KEY = "PUT_YOUR_GOLDEN_KEY_HERE"
USER_AGENT = "PUT_YOUR_USER_AGENT_HERE"
REQUESTS_DELAY = 3

TELEGRAM_ADMIN_BOT_TOKEN = "PUT_YOUR_TELEGRAM_BOT_TOKEN_HERE"
TELEGRAM_ADMIN_USER_ID = 123456789

WELCOME_TEXT = "🤖 Привет! Я бот-помощник автоответчик. Напиши /help или !help, чтобы увидеть доступные команды."
HELP_TEXT = (
    "🤖 Доступные команды (можно писать через / или !):\n"
    "/help или !help — список команд\n"
    "/free или !free — список свободных аккаунтов с рейтингом\n"
    "/acc или !acc — данные аккаунта\n"
    "/code или !code — код для входа\n"
    "/time или !time — время аренды\n"
    "/admin или !admin — позвать продавца"
)

STEAM_SIGN_OUT_ENABLED = True
STEAM_SIGN_OUT_NODE_BIN = "node"
STEAM_SIGN_OUT_WORKER_PATH = "steam_sign_out_worker/steam_kick_all_sessions_worker.js"
STEAM_SIGN_OUT_TIMEOUT_SEC = 420

AUTO_RAISE_ENABLED = False
AUTO_RAISE_INTERVAL_SEC = 7200
