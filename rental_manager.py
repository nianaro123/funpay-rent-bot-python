...
class RentalManager:
    ...
    def _format_issue_message(self, good, order_id: str, hours: int, steam_guard_code: str | None) -> str:
        ...
        lines = [
            ...
            "• После окончания времени аренды вас автоматически разлогинит и выкинет из аккаунта через 45 минут — это дополнительное время предусмотрено на случай, если вы оказались в игре и не успели её завершить, чтобы вы успели доиграть.",
        ]
    ...
    def _format_end_message(self, order_id: str) -> str:
        return "\n".join([
            ...
            "Через 45 минут ваша сессия в Steam будет автоматически завершена и вас выкинет из аккаунта!",
